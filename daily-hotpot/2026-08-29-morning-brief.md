# 2026-08-29 时间序列 Agent / Reasoning / Foundation Model 晨间简报

检索时间：2026-08-29 16:22:15 CST，Asia/Shanghai<br>
时间窗口：2026-05-29 至 2026-08-29<br>
优先来源：arXiv 官方 API / `abs` 页面、GitHub 官方 Repo API / 仓库页、GitHub Search、`DailyArXiv` 官方 README、OpenReview / 会议官网补检<br>
检索主题：`time series foundation model`、`time series agent`、`time series reasoning`、`timeseries harness`、`AutoML agent`、`photovoltaic forecasting`

## 今日摘要

- 截至 `2026-08-29 16:22 CST`，没有检到 `2026-08-28` 或 `2026-08-29` 首发、且相关性高于当前主线的时间序列基础模型 / Agent / reasoning 新论文；今天真正需要补入主列表的新高相关条目是 [`TraceBench`](https://arxiv.org/abs/2608.27182)（`2026-08-27`）。
- 基础模型主线没有改写，最高信号仍集中在 [`Causal Analysis for Time Series Foundation Models`](https://arxiv.org/abs/2608.24303)、[`Do Time-Series Foundation Models Pay Off for Industrial Monitoring?`](https://arxiv.org/abs/2608.22968)、[`LiveHouse-TS`](https://arxiv.org/abs/2608.17299) 与 [`Forecast Collapse in Time-Series Foundation Models`](https://arxiv.org/abs/2608.14106)。
- Agent 主线今天更清楚了：近期最重要的不是“LLM 直接做 forecasting”，而是 `controlled evaluation + live benchmark + meta-harness training`。对应条目分别是 [`TraceBench`](https://arxiv.org/abs/2608.27182)、[`TimeSage-EV`](https://arxiv.org/abs/2608.14270) 和 [`MetaCaster`](https://arxiv.org/abs/2608.23473)。
- Reasoning 支线保持收敛：`selective intervention`、`structured evidence`、`concept bottleneck / explanation`、`tool-using recursion` 四条线仍最值得追。今天没有更晚的新主线论文替代 [`ReasonCast`](https://arxiv.org/abs/2608.15291)、[`Structured Frequency-Domain Evidence`](https://arxiv.org/abs/2608.24113)、[`ConceptTS`](https://arxiv.org/abs/2608.21277) 和 [`TimeRLM`](https://arxiv.org/abs/2608.03391)。
- GitHub 侧今天最值得注意的是活跃度更新而不是全新仓库：[`Jesse-dry/AutoML-Agent`](https://github.com/Jesse-dry/AutoML-Agent) 与 [`Neuraxis-Labs/TSFM-Robustness-Benchmark`](https://github.com/Neuraxis-Labs/TSFM-Robustness-Benchmark) 都在 `2026-08-29` 有新的 `push`；直接面向 time-series agent 的新仓库则仍以 [`sriixz/agentic-timeseries`](https://github.com/sriixz/agentic-timeseries)、[`TimeSage-EV`](https://github.com/TimeSage-Series/TimeSage-EV)、[`Xiaoyu-Tao/CastFSR`](https://github.com/Xiaoyu-Tao/CastFSR) 为主。
- 光伏 / 光功率预测支线今天需要补强一条此前容易漏掉的高相关论文：[`PARA-PV`](https://arxiv.org/abs/2607.08079)。这样过去三个月内最值得跟踪的 PV 交叉线更明确地收敛到 `部署导向 pipeline + retrieval / foundation model + 冷启动 TSFM + 低成本边缘学习`。
- `DailyArXiv` 官方 README 当前 `Last update` 仍是 `2026-08-27`。其 `Time Series` 区可补检到 [`Causal Analysis`](https://arxiv.org/abs/2608.24303)、[`Structured Frequency-Domain Evidence`](https://arxiv.org/abs/2608.24113)、[`MetaCaster`](https://arxiv.org/abs/2608.23473)、[`ConceptTS`](https://arxiv.org/abs/2608.21277)；但其中 [`Time-R1`](https://arxiv.org/abs/2506.10630) 与 [`Perseus`](https://arxiv.org/abs/2510.09930) 的 arXiv 首发日期分别是 `2025-06-12` 与 `2025-10-11`，超出三个月窗口，因此只在补检结论中降优先级说明。
- 今天是周六 `2026-08-29`，不是周五，因此本轮不更新周报。

## 0. 检索口径

- 只保留论文 `published` 日期或 GitHub 仓库 `created_at` 落在 `2026-05-29` 至 `2026-08-29` 的条目。
- 论文日期优先采用 arXiv 官方 API / `abs` 页面；GitHub 项目日期优先采用官方 Repo API 的 `created_at`，活跃度补充采用 `pushed_at` 或 `updated_at`。
- `DailyArXiv` README 中展示的日期视作“日报纳入日期”，不能替代 arXiv 官方首发日期；若二者冲突，以 arXiv 官方日期为准。
- OpenReview、ACL / NeurIPS / ICLR / ICML / KDD / AAAI 官方页面本轮用于补检，没有发现比主列表更晚且更高相关的新增项。
...
### [2026-08-10] [naveen-pulivarti/agentic-automl](https://github.com/naveen-pulivarti/agentic-automl)

- 日期：2026-08-10（创建），2026-08-29（最近一次 push）
- 来源：[GitHub Repo API](https://api.github.com/repos/naveen-pulivarti/agentic-automl) / [GitHub 仓库](https://github.com/naveen-pulivarti/agentic-automl)
- 简短摘要：一个 `reasoning-driven` 的 AutoML 原型仓库，主打 tabular model building 的 agentic workflow。
- 相关性判断：中。它不直接面向 time-series，但属于本轮要求跟踪的 `AutoML agent` 观察项，且今天仍有更新。
...
- 日期：2026-07-12（创建），2026-08-29（最近一次 push）
...
- 相关性判断：最高。它同时命中 `time series + AutoML + agent`，而且今天仍有更新，值得继续跟踪。
...
- 简短摘要：系统化测试 `TSFM` edge cases 的 robustness benchmark 工具仓库，今天有新提交。
...
- 相关性判断：最高。它同时命中 `PV forecasting + retrieval + foundation model` 三个关键词，是这三个月里最值得补回主列表的一篇交叉论文。
...
- `DailyArXiv` 官方 README 当前 `Last update` 为 `2026-08-27`；到本轮检索时间 `2026-08-29 16:22:15 CST` 仍未见 `2026-08-28` 或 `2026-08-29` 新快照，因此补检结论以 `2026-08-27` README 为准。
- 在 `Time Series` 区，本轮可稳定确认且与主题直接相关的窗口内条目包括 [`Causal Analysis for Time Series Foundation Models`](https://arxiv.org/abs/2608.24303)、[`Structured Frequency-Domain Evidence for LLM-Based Time-Series Anomaly Detection`](https://arxiv.org/abs/2608.24113)、[`MetaCaster`](https://arxiv.org/abs/2608.23473) 和 [`ConceptTS`](https://arxiv.org/abs/2608.21277)；它们都已并入主列表，不再重复升权。
- [`Time Series Forecasting via Reasoning: A Slow-Thinking Approach with Reinforcement Fine-Tuned LLMs`](https://arxiv.org/abs/2506.10630) 在 README 中以 `Time-R1` 形式出现，README 纳入日期为 `2026-08-24`，但 arXiv 官方首发日期是 `2025-06-12`，超出三个月窗口，因此只保留为“日报补检命中但超窗”的降优先级说明。
- [`Perseus: Interactive Time Series Segmentation with Sparse Supervision via Stateful Memory`](https://arxiv.org/abs/2510.09930) 在 README 中的纳入日期为 `2026-08-21`，但 arXiv 官方首发日期是 `2025-10-11`，同样超窗，因此不进入主列表。
- [`TraceBench`](https://arxiv.org/abs/2608.27182)、[`ReasonCast`](https://arxiv.org/abs/2608.15291)、[`TimeRLM`](https://arxiv.org/abs/2608.03391) 和 [`TimeSage-EV`](https://arxiv.org/abs/2608.14270) 本轮没有在最新 README 的 `Time Series` 区稳定检到；因此本简报仍以 arXiv 官方首发日期和主题相关性作为主排序依据。

## 7. 结论与下一个观察点

- 如果今天只保留最值得优先读的五篇论文，建议依次看：[`TraceBench`](https://arxiv.org/abs/2608.27182)、[`Causal Analysis`](https://arxiv.org/abs/2608.24303)、[`MetaCaster`](https://arxiv.org/abs/2608.23473)、[`ReasonCast`](https://arxiv.org/abs/2608.15291)、[`TimeRLM`](https://arxiv.org/abs/2608.03391)。
- 如果只保留最值得复现的四个仓库，建议依次跟踪：[`TimeSage-EV`](https://github.com/TimeSage-Series/TimeSage-EV)、[`TimeRLM`](https://github.com/OpenTSLM/TimeRLM)、[`TSFM-Robustness-Benchmark`](https://github.com/Neuraxis-Labs/TSFM-Robustness-Benchmark)、[`AutoML-Agent`](https://github.com/Jesse-dry/AutoML-Agent)。
- 光伏支线如果只挑两篇先读，优先看 [`An AI-Based Decision-Support Pipeline for Day-Ahead Photovoltaic Forecasting`](https://arxiv.org/abs/2608.02088) 和 [`PARA-PV`](https://arxiv.org/abs/2607.08079)；如果关注冷启动，再补 [`Physics-Informed Synthetic Histories for Cold-Start Photovoltaic Forecasting`](https://arxiv.org/abs/2606.07457)。
- 下一个最值得盯的增量不是再来一篇泛时序论文，而是四类东西：`TSFM robustness benchmark 的新增代码/leaderboard`、`agent benchmark 是否出现更严格的 controlled evaluation`、`reasoning 是否继续向 selective intervention / structured evidence / recursive tool use 收敛`、`PV 论文是否释放代码、数据协议或真实站点复现实验资产`。
