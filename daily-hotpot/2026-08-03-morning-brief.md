# 2026-08-03 时间序列 Agent / Reasoning / Foundation Model 晨间简报

检索时间：2026-08-03 15:36 CST，Asia/Shanghai  
时间窗口：2026-05-03 至 2026-08-03  
优先来源：arXiv、OpenReview、官方项目页、GitHub 官方仓库页 / GitHub API、Hugging Face 官方页、机构项目页  
...
- 截至 `2026-08-03` 15:36 CST，没有检索到比 `2026-07-31` 更近、且相关性更高的新窗内论文；研究主线仍以 `TSFM post-training / planning / routing / reasoning benchmark` 为核心。
- 基础模型方向仍由 [`LLM as Forecasting Planner`](https://arxiv.org/abs/2607.24892)、[`Foundation Models and Fine-Tuning`](https://arxiv.org/abs/2607.23146)、[`Post-Training in Time Series Foundation Models`](https://arxiv.org/abs/2607.20002) 和 [`Assessing the Operational Viability of Foundation Models for Time Series Forecasting`](https://arxiv.org/abs/2605.24381) 组成最完整的 `planning + adaptation + deployment` 链路。
- Agent / harness 方向没有出现强于 [`TimeClaw`](https://arxiv.org/abs/2606.05404)、[`AION`](https://arxiv.org/abs/2605.25045)、[`TimeRouter`](https://arxiv.org/abs/2606.11625)、[`GenAutoML`](https://arxiv.org/abs/2606.05860) 的新通用框架，但 GitHub 侧补到了 `rashomon-shap`、`omen-agentic-time-series-forecasting` 等中优先级项目。
- Reasoning 方向仍以 [`ClinPRISM`](https://arxiv.org/abs/2607.25947)、[`TSRouter`](https://arxiv.org/abs/2607.08940)、[`TSCognition`](https://arxiv.org/abs/2606.22126)、[`TimeSage-MT`](https://arxiv.org/abs/2606.01498) 为主；`Can LLM Coding Agents Reason About Time Series?` 继续是工具使用能力最直接的实证检查。
- 光伏 / 光功率预测方向没有出现强于 [`PARA-PV`](https://arxiv.org/abs/2607.08079) 与 [`Time series Foundation Models based on Physics-Informed Synthetic Histories for Cold-Start Photovoltaic Forecasting`](https://arxiv.org/abs/2606.07457) 的新窗内代表论文；工程侧可继续跟踪 [`Helios-Forecast`](https://github.com/ReikanYsora/Helios-Forecast)。
- 已复核 [`zezhishao/DailyArXiv`](https://github.com/zezhishao/DailyArXiv)：仓库在 `2026-08-03` 的 README 已更新，`Time Series` 分栏窗内直接命中 `ClinPRISM`、`LLM as Forecasting Planner`、`Foundation Models and Fine-Tuning`、`Post-Training in TSFMs`。

## 0. 检索口径

- 仅保留首次公开日期落在 `2026-05-03` 至 `2026-08-03` 的论文条目。
- 论文日期优先采用 arXiv `Submitted on`、OpenReview `Published`；若聚合页展示的是修订日期，则回退到原始首发日期，并在 `DailyArXiv 补检结论` 中说明。
- GitHub 项目分为两类：
  - `窗内新建项目`：以 GitHub API `created_at` 为准。
  - `窗内高活跃项目`：创建时间早于窗口，但 `pushed_at` 在窗口内且主题强相关。
- GitHub 项目按 `时间序列` 与 `光伏功率预测` 拆分；同一项目若已有官方 GitHub 仓库，则不再额外单列 Hugging Face 镜像。
- OpenReview 已继续补查 `time series reasoning`、`time series foundation model`、`agentic time series` 等关键词；今天没有发现比正文 arXiv 主线更强的新高优先级条目。
- 若无法确认精确首发日期，则标记为 `不确定` 并降低优先级；本次正文未纳入日期不确定条目。
...
- 简短摘要：把文本条件预测写成 `冻结 TSFM 生成候选轨迹 + LLM 作为 policy/value planner 做 MCTS 搜索`，无需重训 TSFM 或 LLM。
- 相关性判断：最高。它直接把 foundation model、reasoning 与 agent planning 放进同一条推理链路。
...
- 相关性判断：最高。它明确指出 TSFM 研究重心已从“预训练底座是否有效”转向“怎样可靠适配到具体场景”。
...
- 相关性判断：高。它直接服务于“是否值得上线 TSFM”的工程决策。
...
#### [2026-07-28] [devika1402/rashomon-shap](https://github.com/devika1402/rashomon-shap)

- 日期：2026-07-28（GitHub API `created_at`）
- 来源：[GitHub 仓库](https://github.com/devika1402/rashomon-shap)
- 简短摘要：研究近优 AutoML 模型集合中 SHAP 解释是否稳定，覆盖 6 个时间序列数据集和 AutoGluon / H2O 两条 AutoML 栈。
- 相关性判断：中高。它不是 agent runtime，但与 `time series + AutoML + model inspection` 的交集明确。
...
#### [2026-07-22] [badass-data-science/omen-agentic-time-series-forecasting](https://github.com/badass-data-science/omen-agentic-time-series-forecasting)

- 日期：2026-07-22（GitHub API `created_at`）
- 来源：[GitHub 仓库](https://github.com/badass-data-science/omen-agentic-time-series-forecasting)
- 简短摘要：面向 agentic forecasting 的平台化仓库，显式使用 MCP / tool 层组织时序预测工作流。
- 相关性判断：中。主题直接贴合，但成熟度和影响力暂时弱于 `TimeCopilot`、`AION`、`TimeClaw`。
...
- 日期：2026-07-08（GitHub API `created_at`）
- 来源：[GitHub 仓库](https://github.com/Naveen-Boddepalli/time-series-autoML) / [GitHub API](https://api.github.com/repos/Naveen-Boddepalli/time-series-autoML)
- 简短摘要：time-series AutoML 的轻量 workflow / UI 工程脚手架。
...
- 来源：[GitHub 仓库](https://github.com/TROUBADOUR000/Awesome-Agentic-Time-Series)
...
- 来源：[GitHub 仓库](https://github.com/UConn-DSIS/TimeRouter)
...
- 日期：2026-06-11（GitHub API `created_at`）
...
- 简短摘要：面向 Home Assistant 的自学习 solar production forecasting 项目，持续维护中，最近一次推送为 `2026-08-02`。
...
- 已核查 [zezhishao/DailyArXiv](https://github.com/zezhishao/DailyArXiv)。GitHub API 显示仓库 `updated_at` 为 `2026-08-02T17:33:58Z`，`pushed_at` 为 `2026-08-02T17:33:46Z`；README 最近一次自动更新提交时间为 `2026-08-02T17:33:45Z`。
- 当前公开 README 的 `Last update` 已更新到 `2026-08-03`。
- 当前 `Time Series` 分栏中，窗口内且与本主题直接相关的命中包括：
  ...
  - [Post-Training in Time Series Foundation Models: A Unifying Framework](https://arxiv.org/abs/2607.20002)（README 常显示为 `2026-07-24`，但 arXiv 首次提交是 `2026-07-22`；正文已按原始日期保留）。
- 当前 README 中仍存在两类“相关但不应按窗内新条目处理”的情况，需降优先级：
  - [Reverso: Efficient Time Series Foundation Models for Zero-shot Forecasting](https://arxiv.org/abs/2602.17634)：README 常显示为 `2026-07-27`，但 arXiv 首发是 `2026-02-19`，超出三个月窗口。
  - [VisTR: Visualizations as Representations for Time-series Table Reasoning](https://arxiv.org/abs/2406.03753)：README 常显示为 `2026-07-26`，但 arXiv 首发是 `2024-06-06`，属于修订日期造成的“伪新条目”。
- `TimeRouter`、`TimeClaw`、`GenAutoML`、`KairosAgent`、`AION`、`TS-Skill` 等正文核心条目并非都能在 `Time Series` 分栏直接命中，因此继续以原始 arXiv 页面和官方项目页为主来源。

## 7. 结论

- 如果优先关心 `TSFM 主线`，今天最值得读的是 `LLM as Forecasting Planner`、`Foundation Models and Fine-Tuning`、`Post-Training in TSFMs`、`Operational Viability`。
- 如果优先关心 `Agent 系统设计`，建议先看 `TimeRouter`、`TimeClaw`、`GenAutoML`、`AION`，再跟踪 `timeseries-mcp`、`mcp-trajectory-evals` 与 `timecopilot`。
...
- 如果优先关心 `光伏 / 光功率预测`，本窗口内最值得持续跟踪的仍然是 `PARA-PV` 与 `Cold-Start Photovoltaic Forecasting`；工程实现可补看 `Helios-Forecast`。
