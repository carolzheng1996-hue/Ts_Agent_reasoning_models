# 2026-08-30 时间序列 Agent / Reasoning / Foundation Model 晨间简报

检索时间：2026-08-30 15:37:50 CST，Asia/Shanghai<br>
时间窗口：2026-05-30 至 2026-08-30<br>
优先来源：arXiv 官方 API / `abs` 页面、GitHub 官方 Repo API / Search API、`DailyArXiv` 官方 README、OpenReview / 会议官网补检<br>
...
- 截至 `2026-08-30 15:37 CST`，没有检到 `2026-08-30` 首发且高于现有主线的时间序列基础模型 / Agent / reasoning / 光伏预测新论文；今天的主变化仍然集中在 GitHub 活跃度与新增泛 AutoML agent 项目。
- 基础模型主线没有被今天的新来源改写，最高信号仍集中在 [`Causal Analysis for Time Series Foundation Models`](https://arxiv.org/abs/2608.24303)、[`Do Time-Series Foundation Models Pay Off for Industrial Monitoring?`](https://arxiv.org/abs/2608.22968)、[`LiveHouse-TS`](https://arxiv.org/abs/2608.17299) 与 [`Forecast Collapse in Time-Series Foundation Models`](https://arxiv.org/abs/2608.14106)。
- Agent 主线也没有更晚的新论文替代昨天的判断，当前最值得继续跟踪的仍是 `controlled evaluation + live benchmark + meta-harness training` 三条线，对应 [`TraceBench`](https://arxiv.org/abs/2608.27182)、[`TimeSage-EV`](https://arxiv.org/abs/2608.14270) 和 [`MetaCaster`](https://arxiv.org/abs/2608.23473)。
- Reasoning 支线保持稳定：`selective intervention`、`structured evidence`、`concept bottleneck / explanation`、`tool-using recursion` 仍是最近三个月最清晰的四条线，没有比 [`ReasonCast`](https://arxiv.org/abs/2608.15291)、[`Structured Frequency-Domain Evidence`](https://arxiv.org/abs/2608.24113)、[`ConceptTS`](https://arxiv.org/abs/2608.21277)、[`TimeRLM`](https://arxiv.org/abs/2608.03391) 更晚且更高优先级的新条目。
- GitHub 侧今天确认到两个有效变化：[`Neuraxis-Labs/TSFM-Robustness-Benchmark`](https://github.com/Neuraxis-Labs/TSFM-Robustness-Benchmark) 最近一次 `push` 已更新到 `2026-08-30`；另有 [`rvitmonisha/automl-researcher`](https://github.com/rvitmonisha/automl-researcher) 于 `2026-08-28` 创建，可作为泛 `AutoML + agent` 观察项补入主列表。
- GitHub Search API 还检到了 [`ashandilgith/timeseries_prediction_agent-`](https://github.com/ashandilgith/timeseries_prediction_agent-)（创建于 `2026-08-30`），但仓库当前 `size=0`、无描述、无代码，因此只作为“有新仓库出现但不具备研究信号”的降优先级备注，不进入主列表。
- 光伏 / 光功率预测支线今天没有更晚的新论文改写排序；过去三个月最值得跟踪的交叉线仍收敛到 `部署导向 pipeline + retrieval / foundation model + 冷启动 TSFM + 低成本边缘学习`。
- `DailyArXiv` 官方 README 当前 `Last update` 仍为 `2026-08-27`。其 `Time Series` 区依旧可补检到 [`Causal Analysis`](https://arxiv.org/abs/2608.24303)、[`Structured Frequency-Domain Evidence`](https://arxiv.org/abs/2608.24113)、[`MetaCaster`](https://arxiv.org/abs/2608.23473)、[`ConceptTS`](https://arxiv.org/abs/2608.21277)；`Time-R1` 与 `Perseus` 继续因 arXiv 官方首发日期超窗而降优先级。
- 今天是周日 `2026-08-30`，不是周五，因此本轮不更新周报。

## 0. 检索口径

- 只保留论文 `published` 日期或 GitHub 仓库 `created_at` 落在 `2026-05-30` 至 `2026-08-30` 的条目。
...
- 相关性判断：中。它不直接面向 time-series，但命中本轮要求跟踪的 `AutoML + agent` 观察项，且是今天新增确认到的窗口内项目。
...
- 相关性判断：中。它不直接面向 time-series，但属于本轮要求跟踪的 `AutoML agent` 观察项，且最近仍有更新。
...
- 简短摘要：系统化测试 `TSFM` edge cases 的 robustness benchmark 工具仓库，今天确认到新的 `2026-08-30` push。
...
- `DailyArXiv` 官方 README 当前 `Last update` 仍为 `2026-08-27`；到本轮检索时间 `2026-08-30 15:37:50 CST` 仍未见 `2026-08-28`、`2026-08-29` 或 `2026-08-30` 新快照，因此补检结论仍以最新 README 为准。
- 在 `Time Series` 区，本轮可稳定确认且与主题直接相关的窗口内条目仍包括 [`Causal Analysis for Time Series Foundation Models`](https://arxiv.org/abs/2608.24303)、[`Structured Frequency-Domain Evidence for LLM-Based Time-Series Anomaly Detection`](https://arxiv.org/abs/2608.24113)、[`MetaCaster`](https://arxiv.org/abs/2608.23473) 和 [`ConceptTS`](https://arxiv.org/abs/2608.21277)；它们都已并入主列表，不再重复升权。
...
- [`QABBA`](https://arxiv.org/abs/2411.15209) 在最新 README 中排位靠前，但其主题更偏 symbolic compression 与 LLM-friendly representation，不直接命中 `Agent / reasoning / foundation model / PV forecasting` 四条主线，因此继续降优先级，不进入主列表。
- GitHub Search API 本轮没有检到高质量、已具备代码信号的新 `timeseries harness` 仓库；最新命中的 [`ashandilgith/timeseries_prediction_agent-`](https://github.com/ashandilgith/timeseries_prediction_agent-) 虽创建于 `2026-08-30`，但当前为空仓库，只保留为低优先级观察项。
