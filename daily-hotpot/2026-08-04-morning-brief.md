...
检索时间：2026-08-04 09:08 CST，Asia/Shanghai  
...
- 截至 `2026-08-04` 09:08 CST，过去 24 小时内没有检索到比 `2026-07-31` 更晚、且高相关的新窗内论文；今天最明确的增量仍是 `2026-07-31` 首发的 [`TFGformer`](https://arxiv.org/abs/2607.29459)，其摘要实际提出了 `CrossRAG` 式检索增强预测框架。
- 基础模型主线已清晰收敛到 `planning + post-training + retrieval + deployment policy`：[`LLM as Forecasting Planner`](https://arxiv.org/abs/2607.24892)、[`Foundation Models and Fine-Tuning`](https://arxiv.org/abs/2607.23146)、[`Post-Training in Time Series Foundation Models`](https://arxiv.org/abs/2607.20002)、[`TFGformer`](https://arxiv.org/abs/2607.29459) 形成了从冻结底座到外部记忆增强的连续链路。
- Agent / harness 方向今天没有出现强于 [`TimeClaw`](https://arxiv.org/abs/2606.05404)、[`AION`](https://arxiv.org/abs/2605.25045)、[`TimeRouter`](https://arxiv.org/abs/2606.11625)、[`GenAutoML`](https://arxiv.org/abs/2606.05860) 的新通用框架；GitHub 侧最新仍以 `TimeCopilot`、`timeseries-mcp`、`mcp-trajectory-evals`、`TSRouter` 等工具层 / runtime 项目最值得跟踪。
- Reasoning 方向仍以 [`ClinPRISM`](https://arxiv.org/abs/2607.25947)、[`TSRouter`](https://arxiv.org/abs/2607.08940)、[`TSCognition`](https://arxiv.org/abs/2606.22126)、[`TimeSage-MT`](https://arxiv.org/abs/2606.01498) 为主；今天没有检出比 `2026-07-28` 更近、且主题更明确的时间序列 reasoning 新论文。
- 光伏 / 光功率预测方向没有出现强于 [`PARA-PV`](https://arxiv.org/abs/2607.08079) 与 [`Time series Foundation Models based on Physics-Informed Synthetic Histories for Cold-Start Photovoltaic Forecasting`](https://arxiv.org/abs/2606.07457) 的新窗内代表论文；工程侧仍可继续跟踪 [`Helios-Forecast`](https://github.com/ReikanYsora/Helios-Forecast)。

## 0. 检索口径

- 仅保留首次公开日期落在 `2026-05-04` 至 `2026-08-04` 的论文条目。
- 论文日期优先采用 arXiv `Submitted on`、OpenReview `Published`；若聚合页使用的是修订日期，则回退到原始首发日期，并在 `DailyArXiv 补检结论` 中说明。
- GitHub 项目分为两类：
...
- 简短摘要：论文标题是 `TFGformer`，但摘要主体实际提出 `CrossRAG` 式检索增强预测框架，用 shape-aware memory、future-consistent contrastive learning 和 cross-attention fusion 为 TSFM 注入外部历史-未来参考对。
- 相关性判断：高。它不是完整 Agent，但非常接近时间序列 foundation model 的 `retrieval memory layer`，对后续时序 Agent 的外部记忆和 reference selection 很关键。
...
- 相关性判断：高。它偏 foundation model 工程而非 Agent，但非常适合观察工业级 TSFM 的落地实现路径。
...
- 简短摘要：面向 Home Assistant 的自学习 solar production forecasting 项目，最近一次推送为 `2026-08-02`。
...
- 已核查 [zezhishao/DailyArXiv](https://github.com/zezhishao/DailyArXiv)。GitHub API 显示仓库 `updated_at` 为 `2026-08-03T18:18:31Z`，公开 README 的 `Last update` 已更新到 `2026-08-04`。
- 当前 `Time Series` 分栏里，窗口内且与本主题直接相关的最新直接命中包括：
...
- 当前 README 里还存在几类需要降优先级或排除的“伪新条目”：
  ...
  - [Reverso: Efficient Time Series Foundation Models for Zero-shot Forecasting](https://arxiv.org/abs/2602.17634)：README 常显示为近期日期，但 arXiv 首发是 `2026-02-19`，超出窗口。
  - [VisTR: Visualizations as Representations for Time-series Table Reasoning](https://arxiv.org/abs/2406.03753)：README 仍可能出现近期修订日期，但 arXiv 首发是 `2024-06-06`，不应按新条目处理。
...
- `TimeRouter`、`TimeClaw`、`GenAutoML`、`KairosAgent`、`AION`、`TS-Skill` 等正文核心条目并非都能在 `Time Series` 分栏直接命中，因此继续以原始 arXiv 页面、官方项目页与 GitHub API 为主来源。

## 7. 结论

- 如果优先关心 `TSFM 主线`，今天最值得读的是 `TFGformer / CrossRAG`、`LLM as Forecasting Planner`、`Foundation Models and Fine-Tuning`、`Post-Training in TSFMs`、`Operational Viability`。
