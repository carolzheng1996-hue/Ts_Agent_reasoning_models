...
检索截止：**2026-09-18 15:37 CST（Asia/Shanghai）**；保留 08:43 早版，本轮以提交 65648fd 为增量基线。窗口：**2026-06-18 至检索时点**，含起始日。论文按官方 v1 日期，修订日期另列；项目按创建日期，推送只表示活动。来源原始日期使用 UTC。早版增量基线为 9 月 17 日含下午增补版（89fbbce），并对历史日报全文去重。今天周五，同时生成 2026-W38 周报；按项目规则保留光伏栏目。

## 今日重点

- **下午补入 3 篇日期已核验研究**：9 月 17 日的 QUALS 与 SETTer，9 月 15 日的天空图像融合光伏预测论文；另新增 1 个经源码抽查的 Agent harness，以及 1 个仅核验 README/压缩包存在的光伏调度线索。检索增强论文出现编号与首发日期冲突，单独降级。

- **早版新增发现 4 篇窗口内论文**：TuiML、TabPFN-3.5、峰值负荷 TSFM 评测、历史片段预测相关性研究。前三篇首发 9 月 15–16 日；最后一篇首发 8 月 24 日、9 月 16 日修订。均不是今日首发。
- **优先看 TuiML 的工具接口与 TabPFN-3.5 的真实时序结果**：前者提供 Agent 可查询的算法契约；后者虽在表格任务表现强，论文中的时序预测仅列第六，不能把表格榜首外推为 TSFM 榜首。
- **早版 GitHub 新增 2 个有实现的观察候选、1 个核心文件为空的脚手架**。新 AutoML 项目有真实 LLM 调用代码，但缺少明确的时序划分约束；AML 评估 harness 值得参考负对照，公开快照不含真实数据回放包。
- 显式 reasoning 继续跟踪昨天已收录的 WaveTLM；未确认更新的高相关首发。TabPFN 的 Thinking 是推理时计算扩展，正文明确不使用 LLM，不能当成语言思维链模型。

## 1. 时间序列基础模型最新研究

### [2026-09-17] QUALS：面向通用预测的语料均衡 — 下午新增，优先阅读

- 日期与来源：v1 **2026-09-17 12:47:17 UTC**；[官方摘要和版本历史](https://arxiv.org/abs/2609.20156)。
- 摘要：用向量量化与均匀分箱区分混合语料中的模式，再按模式可学习性调整采样权重，改善简单与复杂模式训练不均衡。作者报告减少训练预算后仍可提升零样本预测。
- 相关性：**TSFM 预训练数据效率高，Agent 数据选择与训练预算管理中高，显式 reasoning 低**。这是可用于既有模型的数据框架，不能等同于新的模型权重发布。
- 核验：摘要及日期已确认；未读完整实验、未验证代码或复现。应核验语料去重、测试污染及同预算采样对照，不补写摘要没有提供的节省比例。

### [2026-09-17] SETTer — 下午新增，任务专用对照模型

- 日期与来源：v1 **2026-09-17 11:45:32 UTC**；[官方论文](https://arxiv.org/abs/2609.20086)。
- 摘要：以解耦自注意力、混合掩码和可解释结构处理多变量长时预测中的过平滑与复杂依赖，采用单层 Transformer。作者报告在所测 88% 场景优于比较模型。
- 相关性：**Agent 候选预测器中高，基础模型评测对照中，TSFM 本体 / 显式 reasoning 低**。没有跨域大规模预训练证据，不列作新基础模型；可解释结构也不等于推理链。
- 核验：官方摘要和首发日期已确认；未核实验划分、代码和统计显著性，88% 仅是作者对所测场景的报告。
...
#### [2026-09-17] adityaarun2/agent-reliability-bench — 下午新增，评估 harness

- 日期与来源：创建 **22:56:50 UTC**、推送 **23:13:09 UTC**；[仓库](https://github.com/adityaarun2/agent-reliability-bench)、[元数据](https://api.github.com/repos/adityaarun2/agent-reliability-bench)。
- 摘要：在可枚举的合成环境中，分别检查最终答案、工具返回的证据覆盖和多步失败类型。README 的主要 2730 回合结果来自六种模拟策略；另有真实 LLM 适配代码，二者不能混算。
- 相关性：**Agent / harness / reasoning 验收高，直接时序与 TSFM 低**。可用于时序分析 Agent 的工具证据核查，但当前任务不是序列预测。
- 核验：README、文件树、[grounding.py](https://github.com/adityaarun2/agent-reliability-bench/blob/HEAD/src/arb/metrics/grounding.py)和 [llm.py](https://github.com/adityaarun2/agent-reliability-bench/blob/HEAD/src/arb/agents/llm.py)已读。前者按数字/实体值匹配工具事实，并另计必需证据覆盖；后者有实际 SDK 调用。值匹配不能保证时间戳、单位或实体关系正确，迁移时须强化语义；未运行，未独立验证 README 的测试数与真实模型结果。
...
### [2026-09-18] Forecast-Informed DLR / EV / PV Coordination — 下午新增，低优先级线索

- 日期与来源：创建 **00:32:27 UTC**、推送 **00:38:23 UTC**；[仓库与 README](https://github.com/itodoe4solution-ai/Forecast-Informed-Risk-Aware-Coordination-of-Dynamic-Line-Ratings-with-EV-Charging-and-PV)。
- 摘要：README 描述动态线路容量预测、滚动共形校准、风险感知 MPC、充电与光伏灵活性协调，并明确原始外部数据不随仓库发布。
- 相关性：**光伏预测到决策 / 时序评估中高，Agent 工具中，TSFM / reasoning 低**。主预测目标是线路容量，不能当成已验证的新光伏功率预测模型。
- 核验边界：递归树实际只有 README 和约 800 KB ZIP；未解压、未审计包内源码，不能依据 README 所列目录认定实现已核验。论文发表日期**不确定**，不计新论文；仅按可确认的仓库日期列为低优先级线索。

### [2026-08-10] doccodyblue/ha-pvstrings — 已知项目，活动更新

- 日期与来源：创建 **2026-08-10 14:15:43 UTC**，推送 **2026-09-18 06:13:19 UTC**；[仓库](https://github.com/doccodyblue/ha-pvstrings)、[元数据](https://api.github.com/repos/doccodyblue/ha-pvstrings)。
- 摘要：每个光伏组串以 pvlib 物理预测结合学习残差修正，面向 Home Assistant。
- 相关性：**光伏时序预测高，Agent 可调用工具中，TSFM / reasoning 低**。本轮仅复核搜索元数据，没有提交差分或新实验，不把推送时间当功能进步。早版窗口内前六个光伏搜索结果未补出需提升优先级的新建项目。
...
### [2026-09-15] 天空图像与 CNN-BiGRU-Attention 的短期光伏功率预测 — 下午新增发现

- 日期与来源：期刊页面明确 **Published: 15 September 2026**；[Improving the Short-Term Photovoltaic Power Forecasting Accuracy Based on Deep Learning Hybrid Model](https://www.sciepublish.com/article/pii/1221)，DOI 10.70322/sesr.2026.10012。收稿 7 月 2 日、接收 9 月 2 日均不作为公开日期；更早预印本**未确认**。
- 摘要：光流预测云运动，ResNet50 估计辐照，再把预测辐照、气象与历史功率送入经贝叶斯优化的 CNN-BiGRU-Attention，预测下一小时光伏功率。
- 相关性：**光伏功率预测 / 多模态时序高，Agent 可调用流水线中高，基础模型 / 显式 reasoning 低**。其价值在图像到辐照再到功率的模块化连接。
- 核验：已确认出版社日期和摘要；当前可读页面未给出完整实验划分与代码。未验证站点留出、天气信息起报可得性和全流程超参选择，不据摘要高分宣称跨站点优势。

早版光伏补检多为 9 月卷期的出版商条目，在线首发未核验，不依卷期月份认定新论文；未确认比既有主线更新且日期可靠的高相关首发。历史检索研究的 Solar 结果见第 2 栏，不重复计为已验证光伏方法。

## 6. 检索覆盖、日期过滤与局限

下表保留早版检索记录；下午覆盖及新增日期冲突见表后增补。
...
- **arXiv**：[cs.LG recent](https://arxiv.org/list/cs.LG/recent)已显示 9 月 18 日公告、当日 198 条；本轮检查前两页（1–100 条）的标题检索结果，核验 QUALS、SETTer 原始摘要和日期。较大分页请求受限，未遍历余下 98 条，不声称全量覆盖。此前两篇不在历史日报中。
- **DailyArXiv 必检**：[GitHub README 的 Time Series 板块](https://github.com/zezhishao/DailyArXiv#time-series)显示 Last update 9 月 18 日，直接确认 WaveTLM、TuiML、峰值负荷评测、TabPFN-3.5 与历史检索论文，均已在本日主栏收录；QUALS / SETTer 在所取 README 中无匹配。raw 直连超时，网页工具的 raw 缓存却显示 5 月 29 日，已弃用该过时快照；77 行统计沿用早版完整提取，本轮不冒充重新全量统计。
- **日期冲突及超窗降级**：历史检索论文 2608.23221 的 README 9 月 16 日是修订，主栏仍按 8 月 24 日排序；旧稿 2311.15210、2602.02888、2606.08560 不因 9 月修订纳入新首发。独立 arXiv 检索发现 [When Does Retrieval Help Time-Series Forecasting?](https://arxiv.org/abs/2609.20193)：官方页写 v1 **7 月 29 日**，但编号为 2609 且出现在 9 月 18 日公告，首发日期一致性**不确定，降优先级**。摘要研究窗口长度与季节周期如何影响检索收益，**Agent 检索 / TSFM 对照相关性高**；未计入日期已核验新增，也未将其误并为早版的 Which Histories Matter。StableEval 2609.18949 仍显示 8 月 7 日 v1，保留同类冲突标记。
- **GitHub**：四组窗口内搜索 time-series agent / automl agent / harness machine-learning / photovoltaic forecasting 分别返回 155 / 103 / 134 / 52，各看 updated 排序前三项元数据。新增 harness 经源码抽查；光伏协调 ZIP 项目仅作线索。MLH-IoT-Vector-Pipeline（9 月 17 日创建）有 SQL/MCP 配置，但未核真实预测或建模循环，未列精选。既有 AML harness 推送刷新至 9 月 18 日 04:32:48 UTC，仅作活动，不计新发现。
- **光伏与光功率**：新增期刊条目已核发表日；[SCIFORUM stacking ensemble](https://sciforum.net/paper/33668)搜索索引显示 9 月 16 日发布，但详情页读取失败，日期未独立核验，降级为待核线索。该摘要主题为短期光伏预测，直接相关性高，方法与结果未进一步确认，不计已核验新增。没有确认独立的光通信光功率预测新成果；光学性质重建命中不是功率预测，排除。
- **其余覆盖**：AI HOT 同一关键词仍只返回无关芯片贸易线索，未采纳。会议官网、HF 与机构博客的覆盖沿用早版，本次没有重新逐站检索。新增研究均未复现，ZIP 未解压，外部项目未执行。

## 7. 接下来优先看什么

1. **QUALS 与 TuiML**：先核 QUALS 的同预算采样对照；再看 TuiML 工具接口。**TuiML**：用小型时序任务验证工具契约、实验状态与滚动验证是否贯通，重点检查上游排序和预处理。
