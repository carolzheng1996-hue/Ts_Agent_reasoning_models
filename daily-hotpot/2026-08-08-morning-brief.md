# 2026-08-08 时间序列 Agent / Reasoning / Foundation Model 晨间简报

检索时间：2026-08-08 16:55 CST，Asia/Shanghai  
时间窗口：2026-05-08 至 2026-08-08  
...
- 截至 `2026-08-08` 16:55 CST，论文侧仍以 `2026-08-06` 的 [`Align-RAG`](https://arxiv.org/abs/2608.05571) 和 `2026-08-05` 的 [`Personalized Federated Sparse Adaptation of Time-Series Foundation Models`](https://arxiv.org/abs/2608.04695) 为最近且最值得跟踪的 TSFM 新条目；今天未检到比它们更晚、且相关性更高的时间序列 foundation model 新论文。
...
- GitHub 侧今天没有比 `2026-08-06` 更强的新建时间序列 Agent 项目，但值得继续跟踪的最新仓库/更新仍是 [`Bhavin2127/...Photovoltaic...`](https://github.com/Bhavin2127/Day-Ahead-AI-Driven-Photovoltaic-Energy-Forecasting-A-Physics-and-Data-Driven-Approach)、[`OpenTSLM/TimeRLM`](https://github.com/OpenTSLM/TimeRLM)、[`Naveen-Boddepalli/time-series-autoML`](https://github.com/Naveen-Boddepalli/time-series-autoML)、[`Xiaoyu-Tao/CastFSR`](https://github.com/Xiaoyu-Tao/CastFSR)、[`seunghan96/reasoncast`](https://github.com/seunghan96/reasoncast) 和 [`Lkhanaajav/timeseries-mcp`](https://github.com/Lkhanaajav/timeseries-mcp)。
- 光伏功率预测方向今天最值得补入的是 [`AutoPV`](https://www.sciencedirect.com/science/article/pii/S0306261926005039)；它把 `AutoML / NAS` 明确引入 PVPF，与本仓库关注的 `Agent + AutoML + forecasting` 主线耦合最紧。
- `DailyArXiv` 已直接核对最新公开 `README`，其 `Last update` 为 `2026-08-06`。`Time Series` 栏已收录 `TimeRLM`、`FinVerse`、`CastFSR`、`ReasonCast`、`TRACE-TS`、`ClinPRISM` 与 `TS-Reasoner`；其中 `Align-RAG` 尚未检到。`TS-Reasoner` 虽主题相关，但 arXiv 编号 `2510.03519` 对应首次公开已超出三个月窗口，因此仅在补检结论中降优先级说明。

## 0. 检索口径

- 仅保留首次公开日期落在 `2026-05-08` 至 `2026-08-08` 的条目。
- 论文日期优先采用 arXiv / OpenReview / 期刊官方页的公开日期。
- GitHub 项目日期优先采用 GitHub API `created_at`；若仓库创建时间更早但近期仍强相关，则补充 `pushed_at` 作为活跃度日期。
- `DailyArXiv` 补检基于 `zezhishao/DailyArXiv` 仓库当前 `README.md` 直接核对。
- 未能稳定确认日期的条目不纳入正文；本次正文未纳入日期不确定条目。
...
- 直接核对 `zezhishao/DailyArXiv` 当前 `README.md`，公开 `Last update` 为 `2026-08-06`。
- `Time Series` 栏内、且与本主题直接相关并在窗口内的条目包括：
  - `2026-08-04` [`TimeRLM`](https://arxiv.org/abs/2608.03391)
  - `2026-08-04` [`FinVerse`](https://arxiv.org/abs/2608.03259)
  - `2026-08-04` [`CastFSR`](https://arxiv.org/abs/2608.03031)
  - `2026-08-03` [`ReasonCast`](https://arxiv.org/abs/2608.01875)
  - `2026-07-31` [`TRACE-TS`](https://arxiv.org/abs/2608.00200)
  - `2026-07-28` [`ClinPRISM`](https://arxiv.org/abs/2607.25947)
- 截至本次检索，`README` 中未检到 [`Align-RAG`](https://arxiv.org/abs/2608.05571)，因此它仍是今天正文里最值得额外补入的 TSFM 新论文。
- `README` 中检到 [`TS-Reasoner`](https://arxiv.org/abs/2510.03519)，但该 arXiv 编号对应首次公开时间超出本次三个月窗口；同时 OpenReview 上同名工作公开时间为 `2026-03-20`，也不在窗口内，因此仅作降优先级记录，不纳入正文主条目。

## 7. 额外备注

- 今天是周六（`2026-08-08`），按要求本次不生成周报。
- 本次未检到需要以 `日期不确定` 纳入正文的候选条目。
- 本次主条目主要来自 arXiv、GitHub API、ScienceDirect/MDPI 官方页与 DailyArXiv README；未发现窗口内更强的新 Hugging Face 独立项目卡片。
